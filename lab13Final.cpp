...
class myMatrix {
    ...
    vector<vector<T>> values; // Values of the matrix
    ...
    myMatrix operator+(myMatrix m) {
        myMatrix sum_matrix(NN, MM);
        for (int i = 0; i < NN; i++) {
            for (int j = 0; j < MM; j++) {
                sum_matrix.setValue(i, j, m[i][j] + values[i][j]);
            }
        }
        return sum_matrix;
    ...
    myMatrix operator-(myMatrix m) {
        myMatrix residual_matrix(NN, MM);
        for (int i = 0; i < NN; i++) {
            for (int j = 0; j < MM; j++) {
                residual_matrix.setValue(i, j, m[i][j] - values[i][j]);
            }
        }
    ...
    myMatrix operator*(T x) {
        myMatrix product(NN, MM);
        for (int i = 0; i < NN; i++) {
            for (int j = 0; j < MM; j++) {
                product.setValue(i, j, x * values[i][j]);
            }
        }
        return product;
    ...
    myMatrix minor(int y, int x) { // x - номер вычеркиваемого столбца, y - вычеркиваемая строка
        ...
        for (int i = 0; i < NN; i++) {
            ...
            for (int j = 0; j < MM; j++) {
                ...
                m.setValue(row, column, values[i][j]);
                column++;
            }
            row++;
        }
    ...
    T determinant() {
        ...
        int det = 0;
        for (int i = 0; i < NN; i++) {
            det += multiplier * values[0][i] * minor(0, i).determinant();
